...
func GoGet(s *State, url, uri, cookie string) (*int, *int64) {
	var u = strings.Replace(url,"FUZZ",uri,-1) // glc, fuzz word
	return MakeRequest(s, u, cookie)
}
...
func ParseCmdLine() *State {
	...
	s := State{
		...
		NoStatusCodes: IntSet{set: map[int]bool{}},
	...
	flag.StringVar(&codes, "s", "200,204,301,302,307,401,403", "Positive status codes (dir mode only)")
	flag.StringVar(&nocodes, "ns", "", "Negative status codes (dir mode only) (added by glc)")
	...
	if s.Mode == "dir" {
		...
		// status codes are comma separated
		// glc, ignored status codes
		if nocodes != "" {
			for _, c := range strings.Split(nocodes, ",") {
				i, err := strconv.Atoi(c)
				if err != nil {
					fmt.Println("[!] Invalid status code given: ", c)
					valid = false
				} else {
					s.NoStatusCodes.Add(i)
				}
			}
		}
		...
		if valid {
			...
			if !s.NoConnectTest { // glc, skip the first request that try to connect to the host
...
func PrintDirResult(s *State, r *Result) {
	...
	if s.Verbose {
		if !s.NoStatusCodes.Contains(r.Status) { // glc, ignored status codes
	...
	if s.StatusCodes.Contains(r.Status) || s.Verbose {
		if !s.NoStatusCodes.Contains(r.Status) { // glc, ignored status codes
			if s.Expanded {
				output += s.Url
				output = strings.Replace(output,"FUZZ",r.Entity,-1)  // glc, fuzz word
			} else {
				// output += "/" // glc, fuzz word
				output = r.Entity
			}
...
func ShowConfig(state *State) {
	...
	if state != nil {
		...
		if state.Mode == "dir" {
			// glc, ignored status codes
			if state.NoStatusCodes.Stringify() == "" {
				fmt.Printf("[+] Status codes : %s\n", state.StatusCodes.Stringify())
			} else {
				fmt.Printf("[+] Codes ignored : %s\n", state.NoStatusCodes.Stringify())
			}
